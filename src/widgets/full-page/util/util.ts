import { SelectItem } from '@jetbrains/ring-ui-built/components/select/select';
...
export const issueToSelectItem = (it: Issue) => ({ key: it.id, label: it.idReadable, description: it.summary, model: it });
...
export function matchSelectItemByLabelOrDescription<T>(itemToCheck: SelectItem<T>, checkString: string): boolean {
    const normalizedCheck = checkString.trim().toLowerCase()
    if (!normalizedCheck) {
        return true
    }

    const label = typeof itemToCheck.label === 'string' ? itemToCheck.label.toLowerCase() : ''
    const description = typeof itemToCheck.description === 'string' ? itemToCheck.description.toLowerCase() : ''

    return label.includes(normalizedCheck) || description.includes(normalizedCheck)
}
